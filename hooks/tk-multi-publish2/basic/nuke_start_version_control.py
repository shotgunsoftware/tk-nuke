...
class NukeStartVersionControlPlugin(HookBaseClass):
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "Nuke '%s' plugin rejected the current session..." % (self.name,)
                )
                self.logger.info("  There is already a version number in the file...")
                self.logger.info("  Nuke file path: %s" % (path,))
    ...
    def publish(self, settings, item):
        ...
        self.logger.info("A version number has been added to the Nuke script...")

...
class TestStartup(TankTestBase):
    ...
    def _test_envionment_path_append(self, env_key, generate_test_env, dcc_path, args):
        """
        Tests the generation of passed(env_key) environment variable, to make
        sure that the toolkit startup paths are correctly appended to any pre-existing paths.
        Checks against pre-existing and non-existent scenarios.

        :param env_key str: name of the environment variable, to which our toolkit startup paths should be appended
        :param generate_test_env: a callable method that will generate the appropriate environment
        :param dcc_path: The path to the DCC
        :param args: The Args that will be past to the DCC
        :return: None
        """
        # this loop provides us with a test for pre-existing and non-existent environment paths.
        for existing_path in [None,"/existing/path/test/"]:

            # set the path, or remove the environment variable altogether
            if existing_path:
                os.environ[env_key] = existing_path
            else:
                os.environ.pop(env_key, None)

            # get the environment paths generated by the test
            environment = generate_test_env(is_classic=True)

            # now get the environment generated by the production actual code
            nuke_launcher = sgtk.platform.create_engine_launcher(
                self.tk, sgtk.context.create_empty(self.tk), "tk-nuke-classic", ["10.0v5"]
            )

            launch_info = nuke_launcher.prepare_launch( dcc_path, args, None)

            self.assertEqual(launch_info.environment[env_key], environment[env_key])
    ...
    def test_nuke_path_append(self):
        ...
        :return: None
        """
        # get the environment generated by the tests ready to compare against the startup code

        self._test_envionment_path_append('NUKE_PATH',self._get_nuke_environment, "Nuke.exe", "")
    ...
    def test_nuke_studio_path_append(self):
        ...
        :return: None
        """
        self._test_envionment_path_append('HIERO_PLUGIN_PATH', self._get_hiero_environment, "Nuke.exe", "--studio")
    ...
    def test_hiero_path_append(self):
        ...
        :return: None
        """
        self._test_envionment_path_append('HIERO_PLUGIN_PATH', self._get_hiero_environment, "Nuke.exe", "--hiero")
